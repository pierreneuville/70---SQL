...
my_phases as (
	...
	,FIRST_VALUE(reason_code) OVER (PARTITION BY person_id ORDER BY person_id, effective_start_date) AS first_reason
	,LAST_VALUE(effective_start_date) OVER (PARTITION BY person_id ORDER BY person_id, effective_start_date) AS last_date_ass
	,LAST_VALUE(reason_code) OVER (PARTITION BY person_id ORDER BY person_id, effective_start_date) AS last_reason
...
,first_name
,CASE 
	WHEN last_situation is null 
		and effective_start_date = first_date_ass and effective_start_date <= end_of_Q1
		and last_reason = 'MIG'
	THEN begin_year /*Date à passer en param*/
	ELSE effective_start_date
END as effective_start_date
,CASE 
	WHEN last_situation is null 
		and effective_end_date = last_date_ass and effective_end_date >= end_of_Q3
		and last_reason = 'MOG'
	THEN end_of_year /*Date à passer en param*/
	ELSE effective_end_date
END as effective_end_date
--,first_eff_date
--,next_eff_start_date
...
/*Vérifier ce qui se passe pour un collab sorti de CASA ES en Janvier et revenu*/
/*Ajouter la liste des valeurs de filtre, BU pour les entités*/